...
MAX_BATCH_MSG_COUNT = 100000 #10W
...
def pull_pubsub_msg(keyfile, projectid, pull_subscription_name):
    ...
    try:
        logging.info("strart pull data from subscriptions:" + pull_subscription_name)
        ...
        while empty == False and msgCount < MAX_BATCH_MSG_COUNT:
            ...
            if receivedMessages:
                ...
                #logging.warning("ack_ids top 1 ids:{}".format(", ".join(ack_ids[:3])))
                result.extend(datas)
            else:
                empty = True
